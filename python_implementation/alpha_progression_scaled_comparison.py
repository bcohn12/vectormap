...
def mat_muscle_index(muscle_num):
    return muscle_num+5
def save_baseplots_a_prog(C_mat, muscles_of_interest, alpha_vals, grid_shape):
    """
    ...
    for muscle in muscles_of_interest:
        ...
        for alpha in alpha_vals:
            print(alpha)
            mat = C_mat['C'][1,alpha] #set to 1 to only select the lowerbound.
...
save_baseplots_a_prog(C_normal, muscles_of_interest, alpha_vals)
save_baseplots_a_prog(C_scaled, muscles_of_interest, alpha_vals)
